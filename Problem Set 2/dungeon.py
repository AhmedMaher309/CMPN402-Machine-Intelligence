...
class DungeonState:
    ...
    def __str__(self) -> str:
        ...
        return header + '\n'.join(
            ''.join(position_to_str(Point(x, y)) for x in range(self.layout.width)) for y in range(self.layout.height))
...
class DungeonGame(Game[DungeonState, Direction]):
    ...
    def get_actions(self, state: DungeonState) -> Iterable[Direction]:
        if state.turn == 0:
            ...
            monster_locations = {monster.position for i, monster in enumerate(state.monsters) if
                                 i != index and monster.alive}
            ...
            return [direction for direction, position in positions if
                    position in state.layout.walkable and position not in monster_locations]
    ...
    def get_successor(self, state: DungeonState, action: Direction) -> DungeonState:
        ...
        if current_turn == 0:
            ...
            monsters_at_player = [monster for monster in state.monsters if
                                  monster.position == new_position and monster.alive]
...
def dungeon_heuristic(game: DungeonGame, state: DungeonState, agent: int) -> float:
    ...
    if alive_monsters:
        nearest_monster = min(
            path_length(compute_path(game, state.player.position, monster.position)) for monster in alive_monsters)
    else:
    ...
    if state.player.inventory.keys == 0:
        ...
        if danger <= state.player.inventory.daggers:
            ...
            if danger <= 0:
                value += area  # No danger bonus
            elif nearest_monster < 2:
                value -= area * area  # penalize being too near to a monster
