...
class SudokuProblem(Problem):
    ...
    clues: Dict[
        str, int]  # A dictionary of the clues (the fixed values that are already defined in the puzzle). This is needed for printing only.
        ...
        for pair in var_fixed_pairs:
            for var_list, fixed_list in zip(*pair):
                for index, variable in enumerate(var_list):
                    constraints.extend(
                        UnaryConstraint(variable, unary_not_equal_condition(fixed)) for fixed in fixed_list)
                    constraints.extend(
                        BinaryConstraint((variable, other), not_equal_condition) for other in var_list[index + 1:])
