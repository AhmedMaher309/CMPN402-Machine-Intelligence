...
def train_q_learning(args: argparse.Namespace):
    ...
    if agent_type == "q_learning":
        ...
        agent = ApproximateQLearningAgent(GridFeatureExtractor(), ACTIONS, args.discount, args.epsilon,
                                          args.learning_rate)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("agent", type=str,
                        choices=["value_iteration", "policy_iteration", "sarsa", "q_learning", "q_learning_approx"])
    ...
    parser.add_argument("--tolerance", "-t", type=float, default=0,
                        help="the tolerence of the convergence check in Value Iteration")
    ...
    parser.add_argument("--learning-rate", "-lr", type=float, default=0.05,
                        help="the learning rate (For SARSA & Q-Learning Only)")
    parser.add_argument("--noise", "-n", type=float,
                        help="the action noise (if set, overrides the original value from level file)")
    parser.add_argument("--seed", "-s", type=int, default=time.time_ns(),
                        help="the seed value used for training (To ensure reproducibility)")
    parser.add_argument("--verbosity", "-v", type=int, default=0,
                        help="How often to display the training results (0 will display at the end only)")
