...
class GridMDP(MarkovDecisionProcess[Point, Direction]):
    ...
    def to_display_str(self, state: Optional[Point] = None) -> str:
        ...
        cell_str_width = 1 + max(
            max(0 if cell is None else (len(cell[0]) + len(cell[1])) for cell in row) for row in cells)
        cells = [[(("#" * cell_str_width) if cell is None else (
                cell[0] + (' ' * (cell_str_width - len(cell[0]) - len(cell[1]))) + cell[1])) for cell in row] for
                 row in cells]
        separator = '\n' + ('-' * (w * (cell_str_width + 3))) + '\n'
...
class GridEnv(Environment[Point, Direction]):
    ...
    def step(self, action: Direction) -> Tuple[Point, float, bool, Dict]:
        ...
        return (
            ...
            self.mdp.is_terminal(self.current_state),
            # returns whether the new current state is a terminal state or not
